...
suite("notifier: roles absent from the board are ignored", () => {
  const repo = bus();
  setStatus(repo, "stranger", { status: "working", current: "X" });
  restart(repo).scan();
  setStatus(repo, "stranger", { status: "idle", current: "X" });
  eq(restart(repo).scan().length, 0, "off-roster role not watched");
});
    ...
    new Notifier(repo).notifyOrchestrator(
      ...
      (okFlag) => {
        try {
          ...
          eq(dbg.orchestrator, "po", "targeted the tagged orchestrator");
